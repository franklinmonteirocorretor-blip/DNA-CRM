...
-- ==========================================================================
-- 0023 — CENTRAL DE DOCUMENTOS
-- Expande tipos e status, adiciona rastreamento e checklist obrigatorio
-- ==========================================================================
...
create or replace function fn_checklist_obrigatorio(p_cliente_id uuid, p_etapa text)
returns text[] as $$
declare
  obrigatorios text[];
  pendentes text[];
begin
  ...
  select array_agg(tipo) from (
    select unnest(obrigatorios) as tipo
    except
    select tipo from documentos
    where cliente_id = p_cliente_id and deleted_at is null
      and status_validacao = 'VALIDADO' and tipo = any(obrigatorios)
  ) faltantes into pendentes;
  return coalesce(pendentes, array[]::text[]);
end;
$$ language plpgsql stable security definer;

create index if not exists idx_documentos_vencimento on documentos (vencimento) where deleted_at is null and vencimento is not null;
create index if not exists idx_documentos_observacao_trgm on documentos using gin (observacao gin_trgm_ops) where observacao is not null;

-- ==========================================================================
-- FIM DO RESET COMPLETO. Todas as 23 migrations aplicadas com correcoes.
-- ==========================================================================
