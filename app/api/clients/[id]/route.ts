...
export async function GET(
  ...
  const [client, events, appointments, documents, sources, sales] =
    await Promise.all([
      ...
      supabase
        .from("client_sources")
        .select(
          "origin_type,origin_detail,project_interest,source_file,created_at",
        )
        .eq("client_id", id)
        .order("created_at", { ascending: false }),
      supabase
        .from("sales")
        .select(
          "id,vgv,status,closed_at,payment_date,cancelled_at,cancellation_reason,cancellation_notes,cancellation_actor,projects(name,builders(name))",
        )
        .eq("client_id", id)
  ...
  const error = [events, appointments, documents, sources, sales].find(
    (result) => result.error,
  )?.error;
  if (error)
...
export async function PATCH(
  ...
  await supabase.from("client_events").insert({
    ...
    title: "Ficha do cliente atualizada",
    description: "Dados de qualificação e cadastro atualizados na ficha única.",
  });
