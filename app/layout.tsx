...
export default function RootLayout({ children }: Readonly<{ children: React.ReactNode }>) {
  ...
  return (
    <html lang="pt-BR">
      <head>
        <script dangerouslySetInnerHTML={{ __html: `try{const p=localStorage.getItem('monteiro-profile-photo');if(p)document.documentElement.style.setProperty('--crm-profile-photo','url('+p+')')}catch(e){}` }} />
      </head>
      <body><ProfilePhotoProvider />{children}</body>
    </html>
  );
}
