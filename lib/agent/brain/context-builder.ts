...
import { SupabaseBuilderCatalogService } from "./builder-catalog-service";
...
export async function buildAgentContext(input: { clientId: number; conversationId?: string; recentMessageLimit?: number }): Promise<AgentContext> {
  ...
  const [clientResult, eventsResult, appointmentsResult, documentsResult, knowledgeResult, summariesResult, messagesResult, conversationResult] = await Promise.all([
    ...
    input.conversationId ? db.from("whatsapp_messages").select("id,direction,message_type,body,provider_timestamp,created_at").eq("conversation_id", input.conversationId).eq("client_id", input.clientId).order("created_at", { ascending: false }).limit(limit) : Promise.resolve({ data: [], error: null }),
    input.conversationId ? db.from("whatsapp_conversations").select("control_mode,provider_identity").eq("id", input.conversationId).eq("client_id", input.clientId).single() : Promise.resolve({ data: null, error: null }),
  ]);
  ...
  const latestText = String(recentMessages.at(-1)?.body || "");
  const catalogRows = latestText ? await new SupabaseBuilderCatalogService().findMentioned(latestText) : [];
  return {
    client,
    ...
    presentedProjects: [],
    catalogMatches: catalogRows.map((row) => ({ id: Number(row.id), name: String(row.name), builderName: Array.isArray(row.builders) ? String(row.builders[0]?.name || "") : String((row.builders as { name?: string } | null)?.name || ""), salePrice: Number(row.sale_price), city: String(row.city || "") })),
