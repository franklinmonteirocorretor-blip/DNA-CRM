...
import { GatewayDispatcherProvider } from "./dispatcher-provider.js";
import { DispatcherWorker } from "./dispatcher-worker.js";

const sessions = new SessionManager();
const dispatcher = new DispatcherWorker(createGatewaySupabaseClient(), new GatewayDispatcherProvider(sessions, config.dispatcherAckTimeoutMs), {
  enabled: config.dispatcherWorkerEnabled, pollMs: config.dispatcherPollMs,
  leaseSeconds: config.dispatcherLeaseSeconds, claimLimit: config.dispatcherClaimLimit,
});
  ...
  try {
    ...
    if (req.method === "GET" && url.pathname === "/health") {
      ...
      return json(res, gatewayReady ? 200 : 503, {
        ...
        ...(operational ? { operational, dispatcher: dispatcher.snapshot(), startupError } : {}),
      });
    }
    ...
    if (req.method === "POST" && action === "messages") {
      if (!config.legacyOutboundEndpointEnabled) return json(res, 409, { error: "Endpoint outbound legado desativado." });
      const input = await body(req) as { phone?: string; text?: string };
      if (!input.phone || !input.text) return json(res, 400, { error: "phone e text obrigatórios." });
      return json(res, 202, await sessions.sendText(id, input.phone, input.text));
    }
...
await sessions.restoreActiveSessions()
  .then(() => { dispatcher.start(); gatewayReady = true; })
  .catch(error => {
