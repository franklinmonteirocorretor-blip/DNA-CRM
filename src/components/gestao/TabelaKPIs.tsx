...
export default function TabelaKPIs({ kpis }: { kpis: GestaoKPI[] }) {
  if (kpis.length === 0) {
    return (
      <section>
        <h2 className="text-lg font-semibold text-gray-900">KPIs Diários</h2>
        <p className="mt-3 text-sm text-gray-400">Nenhum corretor encontrado.</p>
      </section>
    )
  }
          ...
          >
            ...
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <p className="text-sm font-semibold text-gray-900">{kpi.nome}</p>
              ...
              <div className="flex-1 h-3 bg-gray-200 rounded-full overflow-hidden">
                <div
                  className={`h-full rounded-full transition-all ${
                    kpi.percentualDiario >= 100 ? 'bg-emerald-500' : kpi.percentualDiario >= 70 ? 'bg-amber-500' : 'bg-red-500'
                  }`}
                  style={{ width: `${kpi.percentualDiario}%` }}
                />
              </div>
              <span className="text-xs font-bold text-gray-700">{kpi.percentualDiario}%</span>
            </div>
          </div>
        ))}
      </div>
