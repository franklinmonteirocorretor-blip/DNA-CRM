...
export default function FunilGerencial({ etapas }: { etapas: GestaoFunilEtapa[] }) {
  if (etapas.length === 0) {
    return (
      <section>
        <h2 className="text-lg font-semibold text-gray-900">Funil Gerencial</h2>
        <p className="mt-3 text-sm text-gray-400">Nenhum dado disponível.</p>
      </section>
    )
  }
      ...
      <div className="mt-3 space-y-1.5">
        {etapas.map((e) => (
          <div key={e.etapa} className="flex items-center gap-3">
            <span className="w-28 shrink-0 text-xs text-gray-500">{e.label}</span>
            ...
            {e.taxaConversao !== null && (
              <span className="w-16 text-right text-[10px] tabular-nums text-gray-400">
