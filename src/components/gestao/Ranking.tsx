...
function Trofeu({ posicao }: { posicao: number }) {
  ...
  return <span className="text-xs font-bold text-gray-400 w-6 text-center">{posicao}º</span>
}
  ...
  if (itens.length === 0) {
    return (
      <section>
        ...
        <p className="mt-3 text-sm text-gray-400">Nenhum dado de ranking disponível.</p>
      </section>
    )
  }
      ...
      <div className="mt-3 grid grid-cols-3 gap-3">
        {top3.map((item) => (
          ...
          >
            ...
            <p className="mt-1 text-sm font-semibold text-gray-900 truncate">{item.nome}</p>
            <p className="text-2xl font-bold text-gray-700">{item.pontuacao.toLocaleString('pt-BR')}</p>
            ...
            {itens.map((item) => (
              <tr
                ...
                className={`border-b border-gray-100 ${
                  item.posicao <= 3 ? 'font-semibold' : ''
                }`}
              >
                <td className="py-2">
                  <Trofeu posicao={item.posicao} />
                </td>
                <td className="py-2 text-gray-900">{item.nome}</td>
