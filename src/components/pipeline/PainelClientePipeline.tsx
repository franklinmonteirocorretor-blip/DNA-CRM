...
export default function PainelClientePipeline({
  ...
  if (carregando) {
    return (
      <div className="fixed inset-y-0 right-0 w-96 bg-white dark:bg-gray-800 shadow-2xl border-l border-gray-200 dark:border-gray-600 z-50 p-6">
        ...
        <div className="p-4 space-y-4 text-sm">
          ...
          <div className="flex gap-2">
            ...
            <Link
              ...
              className="flex-1 rounded-md border border-gray-300 dark:border-gray-600 px-3 py-2 text-xs font-semibold text-gray-700 dark:text-gray-300 text-center hover:bg-gray-50 dark:hover:bg-gray-700"
            >
              Abrir
            </Link>
          </div>
          ...
          {proximas.length > 0 && (
            <div>
              <p className="text-xs font-semibold text-gray-700 dark:text-gray-300 mb-1.5">Avançar para:</p>
              <div className="flex flex-wrap gap-1.5">
                {proximas.map((etapa) => (
                  <button
                    key={etapa}
                    onClick={() => avancarEtapa(etapa)}
                    className="rounded-md bg-gray-100 dark:bg-gray-800 px-2.5 py-1 text-[10px] font-medium text-gray-700 dark:text-gray-300 hover:bg-blue-100 hover:text-blue-700 transition"
                  >
          ...
          {dados.documentos.length > 0 && (
            <div>
              <p className="text-xs font-semibold text-gray-700 dark:text-gray-300 mb-1">Documentos</p>
              <div className="space-y-1">
                {dados.documentos.map((d) => (
                  <div key={d.id} className="flex justify-between text-[10px]">
                    <span className="text-gray-600 dark:text-gray-400">{d.tipo}</span>
            ...
            <div>
              ...
              <div className="space-y-1">
                {dados.agendamentos.slice(0, 5).map((a) => (
                  <div key={a.id} className="text-[10px] text-gray-500 dark:text-gray-400">
          ...
          {dados.atividadesRecentes.length > 0 && (
            <div>
              <p className="text-xs font-semibold text-gray-700 dark:text-gray-300 mb-1">Atividades recentes</p>
              <div className="space-y-1">
          ...
          {dados.historicoEtapas.length > 0 && (
            <div>
              <p className="text-xs font-semibold text-gray-700 dark:text-gray-300 mb-1">Histórico de etapas</p>
              <div className="space-y-1">
...
function Info({ linha, valor, cor }: { linha: string; valor: string; cor?: string }) {
  return (
    <div className="flex justify-between">
      <span className="text-gray-400 dark:text-gray-500">{linha}</span>
      <span className={`font-medium text-gray-700 dark:text-gray-300 truncate max-w-[180px] text-right ${cor ?? ''}`}>{valor}</span>
    </div>
  )
}