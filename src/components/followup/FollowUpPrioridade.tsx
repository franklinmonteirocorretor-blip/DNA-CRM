...
export default function FollowUpPrioridade() {
  ...
  return (
    <div className="rounded-lg bg-white p-5 shadow-sm">
      <h2 className="text-sm font-semibold uppercase tracking-wide text-gray-400">Score de Prioridade</h2>
      <p className="mt-1 text-xs text-gray-400">
        Cada cliente recebe uma pontuação de 0–1000. Quanto maior, mais urgente.
      </p>
      <div className="mt-3 grid grid-cols-1 gap-1.5 sm:grid-cols-2">
        {regras.map((r) => (
          <div key={r.condicao} className="flex items-center justify-between rounded bg-gray-50 px-3 py-2 text-xs">
            <span className="text-gray-600">{r.condicao}</span>
