      ...
      {datas.map(data => (
        <div key={data}>
          <div className="mb-3 flex items-center gap-3">
            ...
            <span className="text-[10px] font-medium uppercase text-gray-400">{data}</span>
            <div className="h-px flex-1 bg-gray-200" />
          </div>
            ...
            {agrupadasPorData[data].map(m => (
              <div key={m.id} className="flex items-start gap-3 px-4">
                <div className={`mt-1 h-2 w-2 rounded-full ${
                  m.remetente === 'usuario' ? 'bg-blue-500' :
                  m.remetente === 'cliente' ? 'bg-green-500' : 'bg-gray-400'
                }`} />
                <div className="min-w-0 flex-1">
                  ...
                  <p className="mt-0.5 text-sm text-gray-900 whitespace-pre-wrap">{m.texto}</p>
                  {m.media_url && (
